...
Output Strictly JSON if action == "search":
...
Output Strictly JSON if action == "answer":

{
  "action": "answer",
  "reasoning": "why this decision was made",
  "answer": "logical text answer to user"
}

Use double backslash for special symbols in the answer
...
Если запрос пользователя не имеет отношения к поиску, ответь ему что-нибудь соответсвующее его сообщению. "action": "answer".

"""+search_api_guide + output_format_prompt
...
Если готов формировать ответ, делай по следующим правилам:
  Правила:
...
EVALUATE_DECIDE_PROMP = """Ты — эксперт по итеративному поиску в RAG системе по транскриптам конференций. 
Твоя задача — анализировать ход предыдущих поисков и решать, нужно ли выполнять дополнительный поиск (action: search) или информации уже достаточно (action: answer).
